			...
			return $(this).each(function (field, index) {
                ...
                if(options.hasChange){
                    /*$(this).on("keypress",function(event){
                        if(event.keyCode == 13){
                            options.hasChange(event.keyCode);
                        }
                    });*/
                    /*$(this).on("change",function(event){
                            options.hasChange(event);
                    });*/
                }
				// Common Types
					...
					case 'hex':
						...
						$(this)
							...
							.on('change', function (event) {
                                ...
                                if(options.hasChange){
                                    options.hasChange(event.target.value);
                                }
								// check validity
								if (this.value != '' && !checkType(this.value)) $(this).val(options.min != null ? options.min : '');								
							});
						if ($(this).val() == '' && options.min != null && options.value == null) $(this).val(options.min);
						if ($(this).val() == '' && options.value != null) $(this).val(options.value);
						if (options.prefix != '') {
							$(this).before(
